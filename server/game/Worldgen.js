...
function buildWalls() {
  ...
  for (const k in C.SPAR_ROOMS) {
    ...
    // Spar obstacles (e.g. showdown cover blocks)
    if (r.obstacles) for (const o of r.obstacles) walls.push({...o});
  }
...
function isSafe(x, y) {
  // Spawn safe bubble
  const sz = C.LOBBY.SAFE_ZONE;
  if (sz && x >= sz.x && x <= sz.x + sz.w && y >= sz.y && y <= sz.y + sz.h) return true;
  // Trade room
  if (C.TRADE_ROOM?.SAFE) {
    const tr = C.TRADE_ROOM;
    if (x >= tr.x && x <= tr.x + tr.w && y >= tr.y && y <= tr.y + tr.h) return true;
  }
  // Upper lounge (sky lounge)
  if (C.UPPER_LOUNGE?.SAFE) {
    const u = C.UPPER_LOUNGE;
    if (x >= u.x && x <= u.x + u.w && y >= u.y && y <= u.y + u.h) return true;
  }
  return false;
}

module.exports = { buildWalls, zoneOf, isSafe };
