...
class GameRoom {
  ...
  _tick() {
    ...
    for (const b of this.bullets) {
      ...
      for (const [sid, p] of this.players) {
        ...
        const bothVault = p.zone.type === 'vault' && shooter.zone.type === 'vault';        if (!inSameSpar && !bothPK && !bothVault) continue;
  ...
  _processMelee(swing, owner) {
    ...
    for (const [sid, p] of this.players) {
      ...
      const bothVault = p.zone.type === 'vault' && owner.zone.type === 'vault';      if (!inSameSpar && !bothPK && !bothVault) continue;
      if (inSameSpar && p.matchKey === 'twos') {
        const m = this.matches['twos'];
