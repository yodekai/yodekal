...
const { buildWalls, zoneOf } = require('./Worldgen');
...
class GameRoom {
  constructor(io) {
    ...
    this.vehicles = C.VEHICLE.spawn.map((p,i) => ({id:'veh_'+i, x:p.x, y:p.y, riderId:null, vx:0, vy:0}));
  ...
  enterEvent(sid) {
    ...
    if (p.vehicleId) this.dismountVehicle(sid);
    this._freeAnySeat(p);
    p.x = C.EVENT_HOUSE.SPAWN.x;
    p.y = C.EVENT_HOUSE.SPAWN.y;
    p.hp = p.maxHP;
    this.io.to(sid).emit('teleported', {zone:'event'});
  }

  exitEvent(sid) {
    const p = this.players.get(sid); if (!p || !p.rateCheck('action')) return;
    if (p.zone.type !== 'event') return;
    const portal = C.LOBBY.EVENT_PORTAL;
    p.x = portal.x + portal.w/2;
    p.y = portal.y + portal.h + 30;
    p.hp = p.maxHP;
    this.io.to(sid).emit('teleported', {zone:'lobby'});
  }
  ...
  giftCoins(sid, targetId, amount) {
    ...
    const tradePoints = [C.LOBBY.TRADE_NPC_A, C.LOBBY.TRADE_NPC_B];
    const nearTrade = tradePoints.some(t => Math.hypot(sender.x - t.x, sender.y - t.y) < 100 && Math.hypot(target.x - t.x, target.y - t.y) < 100);
    if (!nearTrade) {
      this.io.to(sid).emit('tradeResult', {ok:false, msg:'both must stand at the same trade table'});
      return;
    }
  ...
  _tryStartMatch(m) {
    ...
    if (avail.length >= need) {
      ...
      // No countdown — go straight to fight (Darren request)
      this._startFight(m);
    }
  }
  ...
  _endMatch(m, winnerSide, reason) {
    ...
    for (const wid of winners) {
      ...
      if (wp) {
        ...
        if (wp.accountEmail) {
          const c = db.adjustCoins(wp.accountEmail, C.ECONOMY.SPAR_WIN);
  ...
  _tick() {
    ...
    for (const v of this.vehicles) {
      if (v.riderId) {
        ...
        if (z.type === 'spar' || z.type === 'event') this.dismountVehicle(v.riderId);
      }
    }
    ...
    for (const [, p] of this.players) {
      ...
      if (p.vehicleId) {
        ...
        const inp = p.input || {};
        const canShoot = (p.zone.type === 'pk' || p.zone.type === 'vault' || p.zone.type === 'event');
    ...
    for (const [, bot] of this.bots) {
      ...
      // Bot death triggers _onKill via bullet path; opp death also handled there
    }
    ...
    for (const b of this.bullets) {
      ...
      for (const [, bot] of this.bots) {
        ...
        // Bullet from player at this bot only counts if same spar fighter
        ...
        if (Math.hypot(b.x - bot.x, b.y - bot.y) <= C.PLAYER.radius + C.BULLET_RADIUS) {
          ...
          if (!bot.alive) this._onBotKilled(bot, shooterIsP);
          break;
        }
      }
      if (hitBot) continue;
      for (const [sid, p] of this.players) {
        ...
        // Bot-fired bullet hitting player
        ...
        if (botShooter) {
          ...
          if (Math.hypot(b.x - p.x, b.y - p.y) <= C.PLAYER.radius + C.BULLET_RADIUS) {
            ...
            dead.push(b);
            if (!p.alive) this._onPlayerKilledByBot(p, botShooter);
            break;
          }
          continue;
        }
        ...
        const bothEvent = p.zone.type === 'event' && shooter.zone.type === 'event';
        if (!inSameSpar && !bothPK && !bothVault && !bothEvent) continue;
  ...
  _processMelee(swing, owner) {
    ...
    for (const [sid, p] of this.players) {
      ...
      const bothEvent = p.zone.type === 'event' && owner.zone.type === 'event';
      if (!inSameSpar && !bothPK && !bothVault && !bothEvent) continue;
      if (inSameSpar && p.matchKey === 'twos') {
        const m = this.matches['twos'];
  ...
  _onKill(victim, killer, inSameSpar) {
    if (victim.zone.type === 'pk' || victim.zone.type === 'event') {
  ...
  _broadcastSnap() {
    ...
    const vehs = this.vehicles.map(v => ({id:v.id, x:v.x, y:v.y, riderId:v.riderId}));
