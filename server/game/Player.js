...
class Player {
  ...
  tick(dt, walls) {
    ...
    if (!this.alive) {
      if (this.zone.type === 'pk' || this.zone.type === 'vault' || this.zone.type === 'event') {
        this.respawnTimer -= dt;
        if (this.respawnTimer <= 0) {
          let sp;
          if (this.zone.type === 'pk') sp = C.PK.spawns[Math.floor(Math.random() * C.PK.spawns.length)];
          else if (this.zone.type === 'vault') sp = C.VAULT_ZONE.spawns[Math.floor(Math.random() * C.VAULT_ZONE.spawns.length)];
          else sp = C.EVENT_HOUSE.SPAWN;
    ...
    // Block entry to spar room unless fighter — push hard south
    if (this.zone.type === 'spar' && this.role !== 'fighter') {
      const r = this.zone.room;
      this.y = r.y + r.h + 24;
      this.zone = zoneOf(this.x, this.y);
    }
    ...
    // Combat allowed in: spar (as fighter), pk, vault, event
