...
const { zoneOf, isSafe } = require('./Worldgen');
...
class Player {
  ...
  tick(dt, walls) {
    ...
    if (!this.alive) {
      // Always release vehicle if dead (fixes stuck-on-bike-after-death glitch)
      if (this.vehicleId) this.vehicleId = null;
      if (this.zone.type === 'pk' || this.zone.type === 'vault') {
        this.respawnTimer -= dt;
        if (this.respawnTimer <= 0) {
          let sp;
          if (this.zone.type === 'vault') {
            // Vault deaths bounce out to right side of PK zone (push them back to fight in)
            const eastSpawns = (C.PK.spawns || []).filter(s => s.x > (C.PK.x + C.PK.w / 2));
            sp = eastSpawns[Math.floor(Math.random() * eastSpawns.length)] || C.PK.spawns[0];
          } else {
            sp = C.PK.spawns[Math.floor(Math.random() * C.PK.spawns.length)];
          }
        ...
        // Killzone (lobby/upper/trade) — respawn at spawn area with full HP
        this.respawnTimer -= dt;
        if (this.respawnTimer <= 0) {
          const sa = C.LOBBY.SPAWN_AREA;
          this.x = sa.x + sa.w/2 + (Math.random()-0.5)*40;
          this.y = sa.y + sa.h/2 + (Math.random()-0.5)*30;
          this.hp = this.maxHP = C.PLAYER.maxHP;
          this.alive = true;
        }
      }
    ...
    if (this.raceMode && C.WATER_RACE?.speedMul) speedMult *= C.WATER_RACE.speedMul;
    ...
    if (this.dashing) {
      ...
      this.velX = 0; this.velY = 0;
    } else if (!this.inSeat && !this.dancing && !this.vehicleId) {
      let mvx = inp.dx, mvy = inp.dy;
      const ml = Math.hypot(mvx, mvy);
      if (ml > 1) { mvx /= ml; mvy /= ml; }
      // Target velocity vector
      const targetSpeed = C.PLAYER.speed * speedMult;
      const tvx = mvx * targetSpeed, tvy = mvy * targetSpeed;
      // Acceleration ramp (poise/weight) — full speed reached over accelTime
      const accel = (C.PLAYER.accelTime || 0.13);
      const k = Math.min(1, dt / accel);
      this.velX += (tvx - this.velX) * k;
      this.velY += (tvy - this.velY) * k;
      this.x += this.velX * dt;
      this.y += this.velY * dt;
    } else {
      this.velX = 0; this.velY = 0;
    ...
    // Combat allowed in: spar (as fighter), pk, vault, OR killzone (lobby outside safe + outside spar match)
    const canFight = (this.zone.type === 'spar' && this.role === 'fighter')
                     || this.zone.type === 'pk'
                     || this.zone.type === 'vault'
                     || (this.zone.type !== 'spar' && !this.matchKey && !isSafe(this.x, this.y));
    if (!canFight) return [];
      ...
      if (wdef.type === 'melee') {
        ...
        for (let p = 0; p < wdef.pellets; p++) {
          ...
          bullets.push({
            ...
            x:this.x + Math.cos(this.angle) * (C.PLAYER.radius - 2),
            y:this.y + Math.sin(this.angle) * (C.PLAYER.radius - 2),
